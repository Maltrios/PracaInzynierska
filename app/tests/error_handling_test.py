...
def test_logging_out_a_logged_out_user(client):
    ...
    response = client.post("/auth/logout", headers={"Authorization": f"Bearer {token['access_token']}"})
    assert response.status_code == 409, response.json == {"detail": "User was already logged out"}
...
def test_upload_buggy_file_at_least_20_rows(client, logged_in_user):
    ...
    assert response.json() == {"error": "Dataset must have at least 20 rows."}

def test_upload_buggy_file_not_found_target_column(client, logged_in_user):
    token, _ = logged_in_user
    ...
    assert response.status_code == 400
    assert response.json() == {"error": "Selected column not found in dataset"}

def test_upload_buggy_file_target_column_with_1_sample(client, logged_in_user):
    token, _ = logged_in_user
    ...
    assert response.status_code == 400
    assert response.json() == {"error": "Each class in target must have at least 2 samples."}

def test_upload_buggy_file_target_column_with_2_unique_classes(client, logged_in_user):
    token, _ = logged_in_user
    ...
    assert response.status_code == 400
    assert response.json() == {"error": "Target column must have at least 2 unique classes."}
...
def test_upload_empty_file(client, logged_in_user):
    ...
    assert response_file.json()["error"] == "Uploaded file is empty"
...
def test_generate_tree_with_bad_json_request(client, logged_in_user):
    token, _ = logged_in_user
...
def test_upload_invalid_file_format(client, logged_in_user):
    ...
    assert response.status_code == 400
    assert response.json() == {"error":"The file must be in CSV format."}
...
def test_csv_file_without_column_name(client, logged_in_user):
    ...
    assert response.status_code == 400
    assert response.json() == {'error': 'The CSV file must contain valid column names in the first row.'}
